...
```{r eval = TRUE, echo = TRUE, warning = FALSE}
 if (requireNamespace("BSgenome.Hsapiens.UCSC.hg19")) {
  ...
  # transcript database
  txdb <- loadDb(txdbFile)
  computeFeatures(fiveUTR_ORFs, RFP, RNA, txdb, faFile, orfFeatures =  TRUE)
}
```
...
Here the different ORFfinding functions will be shown for it's specific use.

If you have a DNAStringSet or a character vector use findORFs.
Remember if using character: if the annotation uses variation of lowercase and 
uppercase, then the first found type will be used. Normally you want to use 
uppercase, since lowercase means that there is a repeat region. 

findORFs will give you only 5' to 3' direction, so if you want both direction,
...
normally only used for genomes.

If you get both (+/-) strand and only want (+) ORFs, do:
```{r eval = TRUE, echo = TRUE}
  res[strandBool(res)]
```
# Finding ORFs on transcripts
If you want to find ORFs in transcripts, use findMapORFs. It supports automatic
exon splitting if transcripts are spliced.
