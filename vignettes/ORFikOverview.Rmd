...
```{r eval = TRUE, echo = TRUE}
# Lets grab all ORFs with the re-annotated TSS
seqs <- extractTranscriptSeqs(
  BSgenome.Hsapiens.UCSC.hg19::Hsapiens, newFiveUTRs)
ORFs <- findMapORFs(newFiveUTRs, seqs)

# Load RiboSeq example data for hg19 heart tissue. 
# Data here is already shifted for p-sites.
RFP <- rtracklayer::import.bed(
  system.file("extdata", "ribo-seq-heart.bed.bgz",
              package = "ORFik"))
# Load RNASeq example data for hg19 heart tissue.
RNA <- rtracklayer::import.bed(
  system.file("extdata", "rna-seq-heart.bed.bgz",
              package = "ORFik"))

#features <- computeFeatures(
#  grl = ORFs, RFP, RNA,  Gtf = txdb,
#  fiveUTRs = fiveUTRs, faFile = Hsapiens)
# View(features)
```
