...
if (requireNamespace("BSgenome.Hsapiens.UCSC.hg19")) {
  ...
  extractTranscriptSeqs(BSgenome.Hsapiens.UCSC.hg19::Hsapiens, 
                                   fiveUTR_ORFs[1])
  # To save as .fasta do, assign to variable (seqs):
  # writeXStringSet(seqs, filepath = "uorfs.fasta")
}
```
...
if (requireNamespace("BSgenome.Hsapiens.UCSC.hg19")) {
  ...
  unlisted_ranges
  test_ranges <- groupGRangesBy(unlisted_ranges) # <- defualt is tx grouping
}
```
...
```{r eval = TRUE, echo = TRUE} 
  ...
  ORFik:::coverageHeatMap(hitMap, scoring = "transcriptNormalized")
```
...
1. You have some fasta file of the genome only. (For procaryotes)
2. You have some fasta file of the genome and a spliced transcriptome
   annotation. (For eucaryotes with splicing)
3. You have a fasta file of transcripts (eucaryotes or procaryotes)
...
If you have a genome and a spliced transcriptome annotation, you must use findMapORFs(). It takes care of the potential problem from the last example, 
that we really want our result in genomic coordinates in the end.

See above section for example: 
Example of finding ORFs in on 5' UTR of hg19

## Procaryote/Circular Genomes and fasta transcriptomes.
