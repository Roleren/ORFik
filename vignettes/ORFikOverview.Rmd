...
`ORFik` main usage is to get Open reading frames compatitable with Bioconductor's GenomicRanges package, combined with Ribo-seq and RNA-seq analysis.
...
txdb <- loadDb(samplefile)
fiveUTRs = fiveUTRsByTranscript(txdb, use.names = TRUE) # <- extract only 5' leaders
cageData <- "./cageData/heartCage.bed" # location of the cage file
...
ORFik contains a big feature set, things like FLOSS, coverage, ORFscore, entropy, 
translational effiency, inside outside score, distance metrics, etc.
...
RFP <- GenomicAlignments:::readGAlignments("./Ribo-seq-heart.bed") # can also be bam
RNA <- GenomicAlignments:::readGAlignments("./RNA-seq-heart.bed") # can also be bam
