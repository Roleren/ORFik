...
7. Utility functions to extend GenomicRanges for faster grouping, spliting, 
tiling etc.  
...
coordinates `ORFik` supportes three main functions
If input is only .fasta file (or BSGenome), use findORFsFasta, it will treat it as prokaryotic genome and find ORFs on the chromosomes assuming no splicing. If .gtf file is also supplied, sequences of transcripts from .gtf file will be extracted, using findMapORFs will search and also map to genome. 
It is also possible to operate on a vector of characters or Biostrings::DNAString or Biostrings::DNAStringSet, then use findORFs that returns a IRangesList grouped by sequence.

We will now provide two examples of finding ORFs, one with character vector and one with DNAStringSet from BSGenome and map them to genome.
...
test_ranges <- findMapORFs(grl,seqs) # <- ORFs are now in test_ranges
...
# run this:
#uORFs <- findMapORFs(changedFiveUTRs,tx_seqs)
#View(uORFs)
```
...
distance between orfs and cds' (?distToCds) etc.
