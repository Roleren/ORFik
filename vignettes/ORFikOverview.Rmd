...
# Standard plots made easy with ORFik

In investigation of ORFs or other interest regions, ORFik can help you make
some standard plots from reads of Ribo-seq, RNA-seq, CAGE-seq, TCP-seq etc.
...
if (requireNamespace("BSgenome.Hsapiens.UCSC.hg19")) {
  ...
  leaderCov <- scaledWindowPositions(leaders, shiftedReads, scaleTo = 100)

  cdsCov <- scaledWindowPositions(cds, shiftedReads, scaleTo = 100)
  
  trailerCov <- scaledWindowPositions(trailers, shiftedReads, scaleTo = 100)
  # bind together
  dt <- rbindlist(list(leaderCov, cdsCov, trailerCov))
  # Now set info columns
  nGenes <- length(cds)
  dt[, `:=` (fraction = "Ribo-seq", feature = c(rep("leaders", 100 * nGenes), 
     rep("cds", 100 * nGenes), rep("trailers", 100 * nGenes)))]
  
  ORFik:::windowCoveragePlot(dt, scoring = "zscore", 
                             title = "Ribo-seq metaplot") 
}
```

We see a big spike close to start of CDS, called the TIS.
Lets look at this region using the pshifting plot.
...
This looks good, lets check the different read lengths as a heatmap
