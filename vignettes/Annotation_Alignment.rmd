...
If you do not have CAGE, you can also just specify some fixed 5' UTR lengths
```{r eval = FALSE, echo = TRUE, message = FALSE}
  txdb <- "/path/to/local/annotation.gtf.db"
  # Not implemented yet!
  #reassigned_txdb <- assignTSSByCage(txdb, cage = NULL, pseudo.length = 100)
...
Collapsing of duplicated reads is supported (all identical reads merged, with a replicat counter in the header), 
to do this, first only trim the reads to remove adapters.
If your reads are < 55 bases long on average, it is a good idea to collapse duplicated reads (much faster alignment and processing). Note in this case the reads are paired end, so then collapsing is very dangerous. But for single end reads, you could do this: 
...
```
Usually, RNA-seq is > 55 bases, so only do this for short reads like Ribo-seq!
