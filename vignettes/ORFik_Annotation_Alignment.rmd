...
### Download libraries
First we need some data to align, if you have in-lab data, you don't need this step, since you already have access to the files.
...
ORFik comes with a SRA run downloader, just specificy the SRR numbers, or a SRA experiment information csv file containing a column called 'Run' We will also only subset to download the 50000 first reads of the library. So you can replicate this faster. We will now show 2 ways of getting data from SRA.

1. Download using metadata table:
the good thing here is that you can specify a project, and it will find all SRR numbers for you, but here we tell it only to return 
the run called SRR453566. 
```{r eval = FALSE, echo = TRUE, message = FALSE}
outdir.fastq <-  "~/Bio_data/raw_data/RNA-seq/Yeast_SRR453566/"

info <- download.SRA.metadata("SRR453566", outdir.fastq)
download.SRA(info, outdir.fastq, subset = 50000)
```
2. Download using direct SRR numbers
```{r eval = FALSE, echo = TRUE, message = FALSE}
outdir.fastq <-  "~/Bio_data/raw_data/RNA-seq/Yeast_SRR453566/"

download.SRA("SRR453566", outdir.fastq, subset = 50000)
```

We now have the RNA-seq run, separated into 2 files, since this is paired end data.

### Aligning the data

ORFik uses the fastp for trimming reads, this also only works on unix (Linux or Mac OS). If you are on windows, or you want to trim the reads yourself, just run the trimming and give the folder with the trimmed reads as input in next step. Also if you are unsure of what the 3' adapter was, run first FASTQC to which adapters are detected. 
...
If you used the fastp (tr step), you will get a pre-alignment QC report. Just like FastQC in html format.
