---
title: "ORFik Annotations&Alignment"
...
```
If you run this function again after you have run this function and indexed the genome once, it will not re-index, but just output the correct object.
...
```{}
mkdir ~/Bio_data/raw_data/RNA-seq/Yeast_SRR453566/
...
ORFik uses the fastp from trimming reads, this only works on linux. So for Mac users, do the trimming step first with your favorit trimmer if needed, then pass the trimmed files as input argument here. 

We need usually 9 arguments:
...
-steps: steps of depletion and alignment wanted:
		...
		On mac you can not do tr, so trim first then pass the trimmed files as input      and uses steps = "ge"..
...
create.experiment(exper = "Chew_13_RFP_Z11",
...
Now verify that the object is correct, then you can try to read it:
```{r eval = FALSE, echo = TRUE, message = FALSE}
df <- read.experiment(
  file = "~/Bio_data/ORFik_experiments/Chew_13_RFP_Z11"
  )
```
If you are not happy with the libtype, stage, replicates and so on for the file, you can edit the ORFik experiment in excel, libre office or another spreadsheet viewer. 

### convert libraries to new formats
If you want optimzed format identical to bam file, use .ofst.
```{r eval = FALSE, echo = TRUE, message = FALSE}
  remove.experiments(df)
  convertLibs(df, type = "ofst")
```
If you want peaks only, use wig files
```{r eval = FALSE, echo = TRUE, message = FALSE}
  remove.experiments(df)
