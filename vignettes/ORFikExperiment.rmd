...
It gives a ~400x speedup in loading and saving, and 50x speedup in coverage calculation.

### bedoc: bed ORFik file with cigar
From the bam file store these columns as text file:
seqname, cigar, start, strand, score (number of identical replicates for that read)

The R object loaded from these files are GAlignments, since cigar is needed.
