...
There are 2 ways to load NGS data, the first one is to load data into an environment.
...
The second way is to load manually, more secure, but also more cumbersome.
```{r eval = FALSE, echo = TRUE, warning=FALSE}
files <- filepath(df, type = "default")
CAGE_loaded_manually <- fimport(files[1])
```

If you use the auto-loading and you have multiple experiments, it might be a chance of non-unique naming, 2 experiments might have a library called cage.
...
Usually you convert to bed or wig files, but ORFik also support 3 formats
...
- 3. QC plots from experiment (correlation, metacoverage, alignment stats, coverage of (5' UTRs, CDS and 3' UTRs), ++)
...
Here 3 ways to run loops for the data for these cases are shown:
...
3. No parallel evaluation (If you do not have a lot of memory)
```{r eval = FALSE, echo = TRUE, warning=FALSE}
files <- filepath(df, type = "pshifted")
cds <- loadRegion(df, "cds")
# Single thread loop
