...
Let's say we have a human experiment, containing RNA-seq, ribo-seq and CAGE.
...
data.frame(template)
```
You see from the template, it excludes files with .bai or .fai, .rdata etc, and only
doing using data of NGS libraries, defined by argument (type).

You can also see it tries to guess library types, stages, replicates, condition etc.
To fix the things it did not find, you either save the file and modify in excel / libre office, or do it directly in R.

Let's update the template to have correct tissue in one of the samples.
```{r eval = TRUE, echo = TRUE}
...
# And now NGS data
outputLibs(df, chrStyle = txdb) # Use txdb for seqlevelsStyle reference
```
By default all libraries are loaded into .GlobalEnv with names decided by
...
If you are only doing 1 experiment, you don't need to add the name, since
there is no possibility of duplicate naming. Then you can do this:
```{r eval = TRUE, echo = TRUE}
...
simplified libraries for plotting etc.
