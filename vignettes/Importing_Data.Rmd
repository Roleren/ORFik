...
For your own analysis, you can download a matching genome and annotation
from Ensembl or RefSeq. This yeast example requires an internet connection
and is not run when building the vignette:

```{r download-yeast-reference, eval = FALSE}
library(ORFik)
...
The runnable examples below use the small synthetic reference included in
ORFik. Copy it to a temporary directory so that indexing and annotation
optimization do not write into the installed package. This reference is
for learning the API, not for biological analysis.

```{r bundled-reference, message = FALSE}
reference_dir <- system.file("extdata/references/homo_sapiens", package = "ORFik")
output_dir <- tempfile("orfik-reference-")
dir.create(output_dir)
reference_files <- c("Homo_sapiens_dummy.fasta", "Homo_sapiens_dummy.gtf")
file.copy(file.path(reference_dir, reference_files), output_dir)
genome <- file.path(output_dir, reference_files[1])
gtf <- file.path(output_dir, reference_files[2])
...
txSeqsFromFa(GRangesList(GRanges("chr1", IRanges(1, 10), "+")), fa, TRUE)
# Load a 10 first bases from chromosome 1 and 2.
txSeqsFromFa(GRangesList(GRanges("chr1", IRanges(1, 10), "+"),
                       GRanges("chr2", IRanges(1, 10), "+")), fa, TRUE)
```
...
```
Increasing the minimum leader length makes the filter more restrictive:
```{r eval = TRUE, echo = TRUE, message = FALSE}
  filterTranscripts(txdb, minFiveUTR = 10, minCDS = 150, minThreeUTR = 0)
```

Some references, including yeast annotations, have missing or very short
leaders. ORFik can add pseudo 5' UTRs to the TxDb when needed. These are
analysis windows, not experimentally supported transcript boundaries;
the source GTF is not changed. The bundled reference already has leaders.
```{r eval = TRUE, echo = TRUE, message = FALSE}
  txdb <- makeTxdbFromGenome(gtf, genome, organism, optimize = TRUE, return = TRUE, 
                             pseudo_5UTRS_if_needed = 100)
  filterTranscripts(txdb, minFiveUTR = 10, minCDS = 150, minThreeUTR = 0)[1:3]
```
For detailed access to single points such as start sites or regions,
...
uORF discovery searches the annotated or inferred leaders. Interpret results
from pseudo leaders with the limitations of those inferred boundaries in mind.
```{r eval = TRUE, echo = TRUE, message = FALSE}
