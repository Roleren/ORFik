...
extract_START_sites_from_CDSs <- function(cds) {
  ...
  cdsStarts <- GRanges(seqnames = Rle(as.vector(unlist(runValue(seqnames(cds))))),
                       ranges = IRanges(cdsFixedStarts, width = 1),
...
extract_STOP_sites_from_CDSs <- function(cds) {
  ...
  cdsEnds <- GRanges(seqnames = Rle(as.vector(unlist(runValue(seqnames(cds))))),
                     ranges = IRanges(cdsFixedEnds, width = 1),
                     strand = strand(as.vector(unlist(runValue(strand(cds))))))
  return(cdsEnds)
}
